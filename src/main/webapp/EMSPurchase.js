...
window.onload = function getProjects1() {
	...
	xhr.open('GET', 'http://192.168.1.201:8080/EMS/EMSPurchaseServlet', true);
...
function ProjectChange() {
	...
	let Data;
	var xhr = new XMLHttpRequest();
	xhr.open('PUT', 'http://192.168.1.201:8080/EMS/EMSPurchaseServlet', true);
	xhr.setRequestHeader('Content-type', 'application/json');
	xhr.onload = function() {
		if (xhr.status === 200) {
			Data = JSON.parse(xhr.responseText);
...
function getIndentList() {
	...
	xhr2.open('PUT', 'http://192.168.1.201:8080/EMS/EMSPurchaseServlet', true);
...
function submitFormToServlet() {
	...
	// specify the servlet URL and HTTP method
	xhr.open('POST', 'http://192.168.1.201:8080/EMS/EMSPurchaseServlet', true);
