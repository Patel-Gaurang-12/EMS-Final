...
document.querySelector('#add_Quatation').classList.remove('disabled');	
	...
	xhr.open('GET', 'http://192.168.1.201:8080/EMS/EMSGRNApprovalPending', true);
...
function getAllPurchaseDetails()
{
	...
	let Data;
	var xhr = new XMLHttpRequest();
		xhr.open('PUT', 'http://192.168.1.201:8080/EMS/EMSGRNApprovalPending', true);
		xhr.setRequestHeader('Content-type', 'application/json');
		xhr.onload = function() {
		if (xhr.status === 200) {
			Data = JSON.parse(xhr.responseText);
...
function submitFormToServlet()
{
	...
	xhr.open('POST', 'http://192.168.1.201:8080/EMS/EMSGRNApprovalPending', true);
