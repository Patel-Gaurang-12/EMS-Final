...
window.onload = () => {
	...
	xhr.open('GET', 'http://192.168.1.201:8080/EMS/EMSStoreServlet', true);
...
function XHRRequestForStore() {
	...
	xhr.open('POST', 'http://192.168.1.201:8080/EMS/EMSStoreServlet', true);
...
document.getElementById("category-id").addEventListener("change", () => {
	...
	let Data;
	var xhr = new XMLHttpRequest();
	xhr.open('PUT', 'http://192.168.1.201:8080/EMS/EMSStoreServlet', true);
	xhr.setRequestHeader('Content-type', 'application/json');
	xhr.onload = function() {
		if (xhr.status === 200) {
